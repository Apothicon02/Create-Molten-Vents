...
public class MoltenCrimsiteBlockEntity extends BlockEntity {
    ...
    private static void spreadBlock(BlockPos pos, WorldGenLevel level) {
        ...
        if (isTouchingCrimsite && !CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) || isTouchingCrimsite && CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) && level.getBlockState(pos).getFluidState().isSource() || isTouchingCrimsite && level.getBlockState(pos).is(MoltenVentsFeatures.Crimsite)) {
    ...
    private static List<BlockPos> getLavaTouching(BlockPos pos, WorldGenLevel level) {
        ...
        for (int i = 0; i < contacts.length; ++i) { //Check each non-diagonal neighbor to see if it is crimsite
            if (!CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) || CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) && level.getBlockState(pos).getFluidState().isSource() || level.getBlockState(contacts[i]).is(MoltenVentsFeatures.Crimsite)) {
