...
public class DormantAsurineBlockEntity extends BlockEntity {
    public DormantAsurineBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(MoltenBlockEntities.DORMANT_ASURINE_BLOCK_ENTITY.get(), blockPos, blockState);
    }
        ...
        if (!level.isClientSide) {
            ...
            if (serverLevel != null) {
                ...
                if (blockAbove.is(Blocks.TNT)) {
                    ...
                    level.setBlock(blockPos, MoltenBlocks.MOLTEN_ASURINE.get().defaultBlockState(), UPDATE_ALL);
                }
            }
        }
    }
