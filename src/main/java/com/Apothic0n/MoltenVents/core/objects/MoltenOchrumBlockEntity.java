...
public class MoltenOchrumBlockEntity extends BlockEntity {
    public MoltenOchrumBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(MoltenBlockEntities.MOLTEN_OCHRUM_BLOCK_ENTITY.get(), blockPos, blockState);
    }

    public static int spreadDistance = 5; //Max 5
    private static final Block RequiredLiquid = BuiltInRegistries.BLOCK.get(new ResourceLocation(CommonConfig.requiredLiquid.get()));
    ...
    private static void spreadBlock(Block requiredLiquid, BlockPos pos, WorldGenLevel level) {
        ...
        Boolean isTouchingOchrum = false;
        for (int i = 0; i < contacts.length; ++i) { //Check each non-diagonal neighbor to see if it is ochrum
            if (contacts[i].is(MoltenBlocks.Ochrum.get()) || contacts[i].is(MoltenBlocks.MOLTEN_OCHRUM.get())) {
                isTouchingOchrum = true;
            }
        }
        if (isTouchingOchrum && !CommonConfig.useSource.get() && level.getBlockState(pos).is(requiredLiquid) || isTouchingOchrum && CommonConfig.useSource.get() && level.getBlockState(pos).is(requiredLiquid) && level.getBlockState(pos).getFluidState().isSource() || isTouchingOchrum && level.getBlockState(pos).is(MoltenBlocks.Ochrum.get())) {
            if (!CommonConfig.useSource.get() && level.getBlockState(pos).is(requiredLiquid) || CommonConfig.useSource.get() && level.getBlockState(pos).is(requiredLiquid) && level.getBlockState(pos).getFluidState().isSource()) {
                level.setBlock(pos, MoltenBlocks.Ochrum.get().defaultBlockState(), UPDATE_ALL);
        ...
        if (lavaBlocks != null && !lavaBlocks.isEmpty()) {
            for (int i = 0; i < lavaBlocks.size(); ++i) {
                ...
                if (!CommonConfig.useSource.get() && level.getBlockState(lavaPos).is(requiredLiquid) || CommonConfig.useSource.get() && level.getBlockState(lavaPos).is(requiredLiquid) && level.getBlockState(lavaPos).getFluidState().isSource()) {
                    level.setBlock(lavaPos, MoltenBlocks.Ochrum.get().defaultBlockState(), UPDATE_ALL);
    ...
    private static List<BlockPos> getLavaTouching(Block requiredLiquid, BlockPos pos, WorldGenLevel level) {
        ...
        for (int i = 0; i < contacts.length; ++i) { //Check each non-diagonal neighbor to see if it is ochrum
            if (!CommonConfig.useSource.get() && level.getBlockState(contacts[i]).is(requiredLiquid) || CommonConfig.useSource.get() && level.getBlockState(contacts[i]).is(requiredLiquid) && level.getBlockState(contacts[i]).getFluidState().isSource() || level.getBlockState(contacts[i]).is(MoltenBlocks.Ochrum.get())) {
