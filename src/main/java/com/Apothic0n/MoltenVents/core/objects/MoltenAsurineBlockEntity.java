...
public class MoltenAsurineBlockEntity extends BlockEntity {
    ...
    private static void spreadBlock(BlockPos pos, WorldGenLevel level) {
        ...
        if (isTouchingAsurine && !CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) || isTouchingAsurine && CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) && level.getBlockState(pos).getFluidState().isSource() || isTouchingAsurine && level.getBlockState(pos).is(MoltenVentsFeatures.Asurine)) {
    ...
    private static List<BlockPos> getLavaTouching(BlockPos pos, WorldGenLevel level) {
        ...
        for (int i = 0; i < contacts.length; ++i) { //Check each non-diagonal neighbor to see if it is asurine
            if (!CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) || CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) && level.getBlockState(pos).getFluidState().isSource() || level.getBlockState(contacts[i]).is(MoltenVentsFeatures.Asurine)) {
