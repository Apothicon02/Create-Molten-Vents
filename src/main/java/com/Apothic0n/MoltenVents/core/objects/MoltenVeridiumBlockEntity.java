...
public class MoltenVeridiumBlockEntity extends BlockEntity {
    ...
    private static void spreadBlock(BlockPos pos, WorldGenLevel level) {
        ...
        if (isTouchingVeridium && !CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) || isTouchingVeridium && CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) && level.getBlockState(pos).getFluidState().isSource() || isTouchingVeridium && level.getBlockState(pos).is(MoltenVentsFeatures.Veridium)) {
    ...
    private static List<BlockPos> getLavaTouching(BlockPos pos, WorldGenLevel level) {
        ...
        for (int i = 0; i < contacts.length; ++i) { //Check each non-diagonal neighbor to see if it is veridium
            if (!CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) || CommonConfig.useLava.get() && level.getBlockState(pos).is(Blocks.LAVA) && level.getBlockState(pos).getFluidState().isSource() || level.getBlockState(contacts[i]).is(MoltenVentsFeatures.Veridium)) {
